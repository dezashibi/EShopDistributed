...
public class ProductService(ProductDbContext dbContext, IBus bus)
{
    ...
    public async Task UpdateProductAsync(Product updatedProduct, Product inputProduct)
    {
        ...
        if (updatedProduct.Price != inputProduct.Price)
        {
            ...
            var ingegrationEvent = new ProductPriceChangedIntegrationEvent
            {
            ...
            await bus.Publish(ingegrationEvent);

        }

        // NOTE:: Here I haven't consider cases that the following database operations might fail

        // update peoduct with new values
    ...
    public async Task<IEnumerable<Product>> SearchProductsAsync(string query)
    {
        return await dbContext.Products
            .Where(prop => prop.Name.Contains(query))
            .ToListAsync();
    }
}
